...
def get_act_scales(model, tokenizer, dataset_path, num_samples, seq_len, b):
    ...
    def stat_input_hook(m, x, y, name):
        ...
        if name in act_scales_1: 
            act_scales_1[name] = torch.max(act_scales_1[name], comming_max)
            # act_scales_1[name] = torch.cat([act_scales_1[name], x], dim=0) 
        
        else: 
            act_scales_1[name] = comming_max
            # act_scales_1[name] = x 
    ...
    def stat_input_hook_2(m, x, y, name):
        ...
        hidden_dim = x.shape[-1] # embedding/channel dimension
        x = x.view(-1, hidden_dim).abs().detach().float().cpu() # [batch x token, channel]
        # comming_max = torch.max(x, dim=0)[0].float().cpu()

        if name in act_scales_2: 
            act_scales_2[name] = act_scales_2[name] + torch.exp(b * (x - act_scales_1[name] )).sum(dim=0)
        
        else: 
            act_scales_2[name] = torch.exp(b * (x - act_scales_1[name] )).sum(dim=0)
    ...
    print("First Pass to get Global Maximum")
    ...
    for name, m in model.named_modules(): 
        if isinstance(m, nn.Linear): 
            hooks.append(
                m.register_forward_hook(functools.partial(stat_input_hook_2, name=name))
            )
    print("Second Pass to get Log Sum Exp")
    for i in tqdm(range(num_samples)): 
        input_ids = tokenizer(
            dataset[i]["text"], return_tensors="pt", max_length=seq_len, truncation=True
        ).input_ids.to(device)
    ...
    for k, v in act_scales_2.items(): 

        act_scales_2[k] = (act_scales_1[k] + torch.log(act_scales_2[k])) * (1/b)

# (w + torch.log(r)) * (1 / 2)
    return act_scales_2

B = [10, 10.5, 11, 11.5, 17.5, 18, 18.5]

for b in B:
    print(f"Now with B = {b}")
    print("==================")
    act_scales = get_act_scales(model, tokenizer, DATASET_PATH, NUM_SAMPLES, SEQ_LEN, b=b)    
    OUTPUT_PATH_SCALES = 'act_scales_logsum/1b/opt-1b_b_' + str(b) + '.pt' 
...
#     results = {} 
#     OUTPUT_PATH_SCALES = 'act_scales_logsum/opt-125m_b_4.pt'
